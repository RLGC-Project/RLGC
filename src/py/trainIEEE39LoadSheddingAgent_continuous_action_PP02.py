...
flags.DEFINE_enum('policy', 'mlp', ['cnn', 'lstm', 'mlp', 'impala_cnn',
                  ...
                  'Policy architecture')
flags.DEFINE_integer('num_timesteps', int(500),
                     'Number of timesteps to run for.')
flags.DEFINE_integer('num_envs', 1,
                     'Number of environments to run in parallel.')
flags.DEFINE_integer('nsteps', 100, 'Number of environment steps per epoch; '
                     'batch size is nsteps * nenv')
flags.DEFINE_integer('noptepochs', 4, 'Number of updates per epoch.')
flags.DEFINE_integer('nminibatches', 1,
...
def train(learning_rate, env, model_path):
    ...
    vec_env = SubprocVecEnv([lambda: env], context=None)
