...
class PowerDynSimEnv(gym.Env):
    ...
    def _reset(self):
        ...
        study_cases = ipss_app.getStudyCases()
        ...
        total_fault_buses = len(ipss_app.getFaultBusCandidates())
        ...
        fault_start_time_ary = transfer1DJavaArray2NumpyArray(ipss_app.getFaultStartTimeCandidates())
        fault_start_time = fault_start_time_ary[np.random.randint(0, len(fault_start_time_ary))]
       
        ftd_candidates = transfer1DJavaArray2NumpyArray(ipss_app.getFaultDurationCandidates())
